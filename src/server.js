...
function resolveGatewayToken() {
  ...
  try {
    ...
    // no persisted token; fall through to generate
  }
  ...
  try {
    ...
    console.warn(`[token] could not persist generated token: ${err}`);
  }
  return generated;
}
...
async function startGateway() {
  ...
  // Sync wrapper token into openclaw.json so the gateway accepts our injected
  // Authorization headers. Fail fast if the write doesn't stick.
  try {
    ...
    if (verify?.gateway?.auth?.token !== OPENCLAW_GATEWAY_TOKEN) {
      throw new Error("token mismatch after write");
    }
  } catch (err) {
    throw new Error(`Failed to sync gateway token to config: ${err}`);
  }
